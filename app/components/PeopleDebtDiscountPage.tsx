...
export default function PeopleDebtDiscountPage({ headerSelector, editId }: Props) {
      ...
      fetch(`/api/vouchers/${editId}`)
        ...
        .then((voucher) => {
          if (voucher) {
            ...
            // Fallback for vouchers that were saved with zero values (like receipt 45)
  ...
  useEffect(() => {
    ...
    checkFn.owner = 'PeopleDebtDiscountPage.tsx';
    (window as any).hasUnsavedChanges = checkFn;
    return () => {
      if ((window as any).hasUnsavedChanges && (window as any).hasUnsavedChanges.owner === 'PeopleDebtDiscountPage.tsx') {
