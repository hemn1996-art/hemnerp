...
export default function MoneyOutPage({ headerSelector, editId }: Props) {
            ...
            if (voucher.versions && Array.isArray(voucher.versions)) {
              for (let i = voucher.versions.length - 1; i >= 0; i--) {
                try {
                  ...
                  if (parsed && parsed.discountCurrencyId) {
                    loadedDiscountCurId = Number(parsed.discountCurrencyId);
                    break;
                  }
                } catch {}
              }
            }
            setDiscountCurrencyId(loadedDiscountCurId);
  ...
  const [discountCurrencyId, setDiscountCurrencyId] = useState<number | undefined>();
  ...
  const filteredAccounts = useMemo(() => {
    ...
    // Keep only unique accounts by ID
  ...
  useEffect(() => {
    ...
    checkFn.owner = 'MoneyOutPage.tsx';
    (window as any).hasUnsavedChanges = checkFn;
    return () => {
      if ((window as any).hasUnsavedChanges && (window as any).hasUnsavedChanges.owner === 'MoneyOutPage.tsx') {
  ...
  function formatCurrencyAmount(value: number, currencyId: number) {
    ...
    if (code === "IQD") {
      return `${absValue.toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 2 })} دینار`;
    }
    return `${symbol} ${absValue.toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 2 })}`;
  }

  function formatCurrencyAmountJSX(
    value: number,
    currencyId: number,
    isNegativeParam?: boolean,
    ...
    const numberJSX = (
      <span>
        ...
        {decimal && decimal !== "0" && decimal !== "00" && (
          <span style={{ fontSize: "0.8em", opacity: 0.85 }}>.{decimal}</span>
        )}
      </span>
    );
    ...
    return (
      <span style={{ display: "inline-flex", flexDirection: "row", alignItems: "baseline", gap: 3 }} dir="ltr">
        ...
        {isIQD ? (
          <>
            {numberJSX}
            <span style={{ fontSize: "0.85em", opacity: 0.85, fontWeight: 700 }}>دینار</span>
          </>
        ) : (
          <>
            <span style={{ fontSize: "0.85em", opacity: 0.85, fontWeight: 700 }}>{symbol}</span>
            {numberJSX}
          </>
        )}
      </span>
        ...
        {activeEntries.map(([curIdText, val]) => {
          ...
          return (
            <span key={curIdText} style={{ color, fontWeight: 900, fontSize: 14 }}>
              {formatCurrencyAmountJSX(val, curId, false, false)}
            </span>
          );
        })}
      </div>
  ...
  function formatCurrencyMapForPrint(map: Record<string, number>, forceType?: "debt" | "paid" | "auto") {
    ...
    return (
      <span style={{ display: "inline-flex", flexWrap: "wrap", alignItems: "center", justifyContent: "center", width: "100%", textAlign: "center", gap: 6 }}>
        {activeEntries.map(([curIdText, val], idx) => {
          ...
          if (forceType === "debt") {
            color = "#dc2626";
          } else if (forceType === "paid") {
            color = "#16a34a";
          } else {
            color = isNegative ? "#dc2626" : (numVal > 0.01 ? "#16a34a" : "#6b7280");
          }
          ...
          return (
            <span key={curIdText} style={{ display: "inline-flex", alignItems: "center", gap: 4 }}>
              ...
              <span style={{ color, fontWeight: 900 }}>
                {formatCurrencyAmountJSX(numVal, curId, false, false)}
              </span>
            </span>
          );
        })}
  ...
  // Helper inside page:
  function getAccountBalanceIncreaseByCurrency(activeTargetCurrencyId?: number) {
  ...
  function getPaidSummaryText() {
    ...
    const isFixedAccountRate = selectedAccount?.exchangeRateType === "FIXED";
    const usdCurrency = currencies.find((c: any) => c.code === "USD") || { id: 1, symbol: "$" };
    const rateVal = (toNumber(exchangeRate) || selectedAccount?.customExchangeRate || 150000) / 100;

    return list
      .map((item: any) => {
        const isItemUsd = item.currencyId === usdCurrency.id;
        if (isFixedAccountRate && !isItemUsd && rateVal > 0) {
          const usdAmount = item.amount / rateVal;
          return `${formatCurrencyAmount(usdAmount, usdCurrency.id)} (${formatCurrencyAmount(item.amount, item.currencyId)})`;
        }
        return formatCurrencyAmount(item.amount, item.currencyId);
      })
      .join(" ، ");
  }
  ...
  function getPaidSummaryJSX() {
    const list = getPaidCurrencies();
    if (list.length === 0) return <span style={{ color: "#6b7280" }}>0</span>;
            ...
            return (
              <Fragment key={idx}>
                <span style={{ color: "#16a34a", fontWeight: 900 }}>
                  {formatCurrencyAmountJSX(usdAmount, usdCurrency.id, false, false)}
                </span>
                <span style={{ position: "absolute", left: 0, fontSize: "0.85em", opacity: 0.85, fontWeight: "bold", color: "#15803d" }} dir="ltr">
                  ({formatCurrencyAmountJSX(item.amount, item.currencyId, false, false)})
                </span>
              </Fragment>
            );
          }
  ...
  function getDiscountSummaryJSX() {
    const rawDiscount = toNumber(discountAmount);
    if (rawDiscount <= 0) return null;
    ...
    const usdCurrency = currencies.find((c: any) => c.code === "USD") || { id: 1, symbol: "$" };
    const rateVal = (toNumber(exchangeRate) || selectedAccount?.customExchangeRate || 150000) / 100;
    const curId = activeDiscountCurrencyId || defaultCurrency.id;
    const isDiscountUsd = curId === usdCurrency.id;

    if (isFixedAccountRate && !isDiscountUsd && rateVal > 0) {
      const usdAmount = rawDiscount / rateVal;
      return (
        <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center", width: "100%" }}>
          <span style={{ color: "#16a34a", fontWeight: 900 }}>
            {formatCurrencyAmountJSX(usdAmount, usdCurrency.id, false, false)}
          ...
          <span style={{ position: "absolute", left: 0, fontSize: "0.85em", opacity: 0.85, fontWeight: "bold", color: "#15803d" }} dir="ltr">
            ({formatCurrencyAmountJSX(rawDiscount, curId, false, false)})
          </span>
        </div>
      );
    }
  ...
  function getTotalPaidInDefaultCurrency() {
    return getPaidCurrencies().reduce((sum, item) => {
      return (
        sum + convertCurrency(item.amount, item.currencyId, defaultCurrency.id)
      );
    }, 0);
  }
  ...
  function validateBeforeSave() {
    if (!accountId) {
      showToast("پسوڵەی پارەی ڕۆشتوو نابێت بێ هەژمار خەزن بکرێت.");
      return false;
    }
  ...
  function applyCashboxDecrease() {
    ...
    if (typeof cashbox.balance === "number") {
      cashbox.balance =
        Number(cashbox.balance || 0) - getTotalPaidInDefaultCurrency();
    }
  }

  function applyAccountBalanceIncrease(action: "keep_credit" | "cross_deduct" | null) {
    if (!accountId) return;
  ...
  function resetReceipt() {
    ...
    setTargetCurrencyId(undefined);
    setDiscountAmount("");
    setDiscountCurrencyId(undefined);
  }
        ...
        if (res) {
          ...
          showToast(
            editId
               ...
               : "پسوڵەی پارەی ڕۆشتوو خەزن کرا ✅ قاسە کەمکرایەوە و قەرزی هەژمار زیادکرایەوە.",
            "success"
          );
        } else {
  ...
  function handleSave() {
    if (isLocked || isSaved || isSaving) {
      showToast("هیچ گۆڕانکارییەک نییە بۆ خەزنکردن.");
      return;
    }
  ...
  function handlePrint(size: "A4" | "A5" = "A4") {
    ...
    openPrintWindow("money-out-print-area");
  }
  ...
  return (
    <div style={page}>
        ...
        <aside style={leftPanel}>
          ...
          <div style={totalsCard}>
            <div style={totalGrid}>
              ...
              {toNumber(discountAmount) > 0 && (
                <StatBox
                  title="داشکاندن"
                  value={`${toNumber(discountAmount).toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 2 })} ${currencies.find((c: any) => c.id === activeDiscountCurrencyId)?.symbol || "$"}`}
                  color="#dc2626"
                />
              )}
            </div>
            ...
            <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 10 }}>
              {currencies.filter((c: any) => c.id === paidCurrencyId || (paidAmounts[c.id] && paidAmounts[c.id].trim() !== "" && parseFloat(paidAmounts[c.id]) !== 0)).map((currency: any) => {
                ...
                const rateInput = isConverted ? (
                  <Field label={isFixedAccountRate ? "ڕەیتی 100 دۆلار (جێگیر)" : "ڕەیتی 100 دۆلار"}>
                    <div style={{ display: "flex", border: isFixedAccountRate ? "1.5px solid #a855f7" : "1px solid #93c5fd", borderRadius: 8, overflow: "hidden", background: isFixedAccountRate ? "#faf5ff" : "#f0f9ff" }}>
                      <FormattedNumberInput
                        value={exchangeRate}
                        disabled={isLocked}
                        onChange={(val) => {
                        ...
                        style={{ flex: 1, minWidth: 0, border: "none", outline: "none", padding: "8px 10px", background: "transparent", cursor: isLocked ? "not-allowed" : "text", color: isFixedAccountRate ? "#7e22ce" : "#1e40af", fontWeight: "bold" }}
                      />
                      <span style={{ border: "none", borderRight: isFixedAccountRate ? "1px solid #d8b4fe" : "1px solid #bfdbfe", background: isFixedAccountRate ? "#f3e8ff" : "#e0f2fe", padding: "0 8px", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: "bold", color: isFixedAccountRate ? "#6b21a8" : "#0369a1", fontSize: "12px", minWidth: "45px" }}>
                        دینار
                      </span>
                    </div>
                  </Field>
              ...
              <Field label="داشکاندن">
                <div style={{ display: "flex", border: "1px solid #d1d5db", borderRadius: 8, overflow: "hidden", width: "100%" }}>
                  <FormattedNumberInput
                    ...
                    placeholder="0"
                    style={{ ...input, border: "none", flex: 1, outline: "none" }}
                  />
                  <select
                    value={activeDiscountCurrencyId}
                    disabled={isLocked}
                    onChange={(e) => {
                      if (blockIfLocked()) return;
          ...
          <div style={sideActions}>
            ...
            <button
              style={{
                ...primaryBtn,
                opacity: (isLocked || isSaved || isSaving) ? 0.55 : 1,
                cursor: (isLocked || isSaved || isSaving) ? "not-allowed" : "pointer",
              }}
              onClick={handleSave}
              disabled={isLocked || isSaved || isSaving}
            >
              {isSaving
                ? "خەریکە خەزن دەکرێت..."
                : (isLocked || isSaved)
                ? (editId ? "نوێکراوەتەوە" : "خەزن کراوە")
                : editId
                ? "نوێکردنەوە"
                : "خەزنکردن"}
            </button>
      ...
      <div id="money-out-print-area" style={printArea}>
        <div style={printPage}>
          <PrintWatermark />
          <PrintHeader />
            ...
            <div style={printInfoGrid}>
              ...
              {printOptions.showReceiptInfo ? (
                <div style={{ ...printInfoBox, width: "100%", minWidth: "220px" }}>
                  ...
                  <PrintInfoLine label="ژمارەی پسوڵە" value={receiptNumber || (originalVoucher?.id ? String(originalVoucher.id) : (editId ? String(editId) : "-"))} />
                  <PrintInfoLine
                    label="بەروار"
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style={{ border: "1px solid #cbd5e1", padding: "10px 14px", textAlign: "center", fontWeight: 900, fontSize: 15 }}>
                  {formatCurrencyMapForPrint(accountBalanceBeforeByCurrency, "debt")}
                </td>
              </tr>

              {/* Row 2: Paid Amount (Green, converted to USD for fixed dollar rate) */}
              <tr>
                <td style={{ border: "1px solid #cbd5e1", padding: "10px 14px", textAlign: "center", fontWeight: 900, color: "#374151", whiteSpace: "nowrap", width: "40%" }}>
                  بڕی پارەی دراو
                </td>
              ...
              {/* Row 3: Discount (Green, inside table) */}
              {toNumber(discountAmount) > 0 && (
                <tr>
                  <td style={{ border: "1px solid #cbd5e1", padding: "10px 14px", textAlign: "center", fontWeight: 900, color: "#374151", whiteSpace: "nowrap", width: "40%" }}>
                    بڕی داشکاندن
                  ...
                  <td style={{ border: "1px solid #cbd5e1", padding: "10px 14px", textAlign: "center", fontWeight: 900, fontSize: 15 }}>
                    {getDiscountSummaryJSX()}
                  </td>
                </tr>
              )}

              {/* Row 4: Current Balance (Centered, same font, no monospace, color-coded) */}
              <tr style={{ background: "#f8fafc" }}>
                <td style={{ border: "1px solid #cbd5e1", padding: "10px 14px", textAlign: "center", fontWeight: 900, color: "#374151", whiteSpace: "nowrap", fontSize: 15, width: "40%" }}>
                  باڵانسی ئێستا
                </td>
                <td style={{ border: "1px solid #cbd5e1", padding: "10px 14px", textAlign: "center", fontWeight: 900, fontSize: 15 }}>
                  {formatCurrencyMapForPrint(editId && selectedAccount ? getAccountBalanceBeforeMap(selectedAccount) : accountBalanceAfterByCurrency)}
                </td>
              </tr>

              {/* Row 5: Exchange Rate (if applicable) */}
              {showRate && (
                <tr style={{ background: "#f8fafc" }}>
                  <td style={{ border: "1px solid #cbd5e1", padding: "8px 14px", textAlign: "center", fontWeight: 900, color: "#475569", whiteSpace: "nowrap", fontSize: 13, width: "40%" }}>
                    ڕەیتی 100 دۆلار
                  </td>
                  <td style={{ border: "1px solid #cbd5e1", padding: "8px 14px", textAlign: "center", fontWeight: 900, fontSize: 13, color: "#1e40af" }}>
      ...
      {excessModalConfig?.isOpen && (
        <div style={modalOverlay}>
          <div style={{ ...confirmBox, width: 500, background: "rgba(255, 255, 255, 0.8)", border: "1px solid rgba(255, 255, 255, 0.3)", backdropFilter: "blur(12px)" }}>
...
const printMoneyBox: CSSProperties = {
  border: "1px solid #e5e7eb",
  padding: 8,
  minHeight: 45,
  marginBottom: 8,
