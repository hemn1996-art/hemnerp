...
export default function MaterialIssuePage({ headerSelector, editId }: Props) {
  ...
  useEffect(() => {
    ...
    checkFn.owner = 'MaterialIssuePage.tsx';
    (window as any).hasUnsavedChanges = checkFn;
    return () => {
      if ((window as any).hasUnsavedChanges && (window as any).hasUnsavedChanges.owner === 'MaterialIssuePage.tsx') {
  ...
  function handleSave() {
    ...
    const totalAmountUSD = rows.reduce((sum: any, row: any) => {
          ...
          {selectedAccount && showAccountInfo && (
            <div style={accountCard}>
              ...
              <InfoRow label="باڵانس">
                {formatCurrencyAmount(selectedAccount.balance || 0, defaultCurrency.id)}
              </InfoRow>
            </div>
          )}
