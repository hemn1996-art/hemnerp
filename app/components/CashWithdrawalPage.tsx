...
export default function CashWithdrawalPage({ headerSelector, editId }: Props) {
  ...
  function getShareholderBalanceAfter(baseMap: Record<string, number>) {
    ...
    // باڵانسی پشک هەمیشە بە دۆلار کەم دەبێتەوە لە کاتی کشانەوەی پارە
  ...
  useEffect(() => {
    ...
    checkFn.owner = 'CashWithdrawalPage.tsx';
    (window as any).hasUnsavedChanges = checkFn;
    return () => {
      if ((window as any).hasUnsavedChanges && (window as any).hasUnsavedChanges.owner === 'CashWithdrawalPage.tsx') {
  ...
  function applyCashboxDecrease() {
    ...
    const withdrawalAmount = toNumber(amount);
    ...
    if (foundBalance) {
      foundBalance.amount = Number(foundBalance.amount || 0) - withdrawalAmount;
    } else {
    ...
    if (typeof cashbox.balance === "number") {
      cashbox.balance = Number(cashbox.balance || 0) - withdrawalAmount;
    }
  }

  function applyShareholderBalanceDecrease() {
    const account = accounts.find((item: any) => item.id === accountId);
    if (!account) return;

    const withdrawalAmount = currentUsdEquivalent;
    const usdKey = String(usdCurrencyId);
    ...
    if (typeof account.shareholderBalance === "number") {
      account.shareholderBalance =
        Number(account.shareholderBalance || 0) - withdrawalAmount;
    } else {
      account.shareholderBalance = -withdrawalAmount;
    }
  }
  ...
  return (
    <div style={page}>
        ...
        <aside style={rightSidebar}>
          ...
          <div style={totalsCard}>
            ...
            {isIQD && toNumber(amount) > 0 && (
              <div style={{ marginTop: 8, padding: "8px 12px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 8, textAlign: "center" }}>
                <span style={{ fontSize: 13, fontWeight: "bold", color: "#991b1b" }}>
          ...
          <div style={emptyMainCard}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div style={{ fontWeight: 800, fontSize: 16, color: "#991b1b" }}>
                پسوڵەی کشانەوەی پارە (Withdrawal)
              </div>
              <button
                type="button"
                onClick={handlePrint}
            ...
            <div style={{ marginTop: 24, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16 }}>
              ...
              <div style={{ background: "white", padding: 14, borderRadius: 10, border: "1px solid #e2e8f0" }}>
                ...
                <div style={{ fontSize: 18, fontWeight: "900", color: "#dc2626", marginTop: 4 }}>
            ...
            {isIQD && toNumber(amount) > 0 && (
              <div style={{ marginTop: 16, background: "#fef2f2", padding: "10px 14px", borderRadius: 8, border: "1px solid #fecaca", color: "#991b1b", fontSize: 13, fontWeight: "bold" }}>
      ...
      <div id="cash-withdrawal-print-area" style={printArea}>
        <div style={printPage}>
          <PrintWatermark />
          <PrintHeader />
