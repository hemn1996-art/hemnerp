...
function getShareholderBalanceAfter(baseMap: Record<string, number>) {
    ...
    result[String(currencyId)] =
      Number(result[String(currencyId)] || 0) - toNumber(amount);
  ...
  function validateBeforeSave() {
    ...
    if (selectedCashbox) {
      const bal = selectedCashbox.balances?.find((item: any) => item.currencyId === currencyId);
  ...
  function applyCashboxDecrease() {
    ...
    if (foundBalance) {
      foundBalance.amount = Number(foundBalance.amount || 0) - withdrawAmount;
    } else {
    ...
    if (typeof cashbox.balance === "number") {
      cashbox.balance = Number(cashbox.balance || 0) - withdrawAmount;
    }
  }
  ...
  function applyShareholderBalanceDecrease() {
    ...
    if (typeof account.shareholderBalance === "number") {
      account.shareholderBalance =
        Number(account.shareholderBalance || 0) - withdrawAmount;
    } else {
      account.shareholderBalance = -withdrawAmount;
    }
  }
      ...
      <div style={pageGrid} className="no-print">
        ...
        <main style={mainContent}>
          <div style={headerCard}>
            {headerSelector ? headerSelector : <h2 style={{ margin: 0 }}>پسوڵەی کشانەوەی پارە</h2>}
