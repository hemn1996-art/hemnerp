...
export default function PurchaseReturnPage({ headerSelector, editId }: Props) {
  ...
  const totalPaidInBase = getTotalPaidInReturnCurrency();
  ...
  useEffect(() => {
    ...
    checkFn.owner = 'PurchaseReturnPage.tsx';
    (window as any).hasUnsavedChanges = checkFn;
    return () => {
      if ((window as any).hasUnsavedChanges && (window as any).hasUnsavedChanges.owner === 'PurchaseReturnPage.tsx') {
  ...
  function getSupplierBalanceReductionByCurrency() {
    const balanceCurrencyId = getSingleAccountBalanceCurrencyId(supplier);
    ...
    const paid = normalizeMoneyMapToSingleCurrency(
    ...
    for (const currencyId of currencyIds) {
      ...
      result[key] = Number(totals[key] || 0) - Number(paid[key] || 0);
    }
  ...
  function getTotalPaidInReturnCurrency() {
  ...
  function chooseProduct(product: ProductLike) {
    ...
    const requestedUnits = firstPackage.quantity || 1;

    if (Number(product.stock || 0) < requestedUnits) {
      showToast(
        `دانەی بەردەستی "${product.name}" پێی ناگات. نابێت کۆگا بچێتە ناقص.`
      );
      return;
    }
    ...
    if (existing) {
      const nextUnits = (toNumber(existing.qty) + 1) * existing.packageQuantity;

      if (nextUnits > existing.availableStock) {
        showToast(
          `دانەی بەردەستی "${existing.productName}" پێی ناگات. نابێت کۆگا بچێتە ناقص.`
        );
        return;
      }
    ...
    const rowCurrencyId =
      product.purchaseCurrencyId || product.costCurrencyId || returnCurrencyId;
    ...
    const newRow: ReturnRow = {
      ...
      code: product.code || "",
      qty: "1",
      returnPrice: String(product.purchasePrice || product.costPrice || 0),
      discount: "",
      note: "",
  ...
  function changeRowPackage(row: ReturnRow, packageName: string) {
    ...
    const newPackageQuantity = selectedPackage?.quantity || 1;
    const nextUnits = toNumber(row.qty) * newPackageQuantity;

    if (nextUnits > row.availableStock) {
      showToast(
        `دانەی بەردەستی "${row.productName}" پێی ناگات. نابێت کۆگا بچێتە ناقص.`
      );
      return;
    }
  ...
  function validateBeforeSave() {
    if (!supplierId) {
      showToast("پسوڵەی گەڕانەوەی کڕین نابێت بێ دابینکەر خەزن بکرێت.");
      return false;
    }
    ...
    for (const row of rows) {
      ...
      if (units > row.availableStock) {
        showToast(
          `ناتوانیت ${units} دانە لە "${row.productName}" بگەڕێنیتەوە، چونکە تەنها ${row.availableStock} دانە بەردەستە.`
        );
        return false;
      }
  ...
  function applyStockDecrease() {
    for (const row of rows) {
      ...
      product.stock = Number(product.stock || 0) - units;
    }
  }
  ...
  function applyCashboxIncrease() {
    ...
    const paidInBase = getTotalPaidInReturnCurrency();
  ...
  function applySupplierBalanceReduction() {
    ...
    <div style={page}>
      ...
      <div style={pageGrid} className="no-print">
          ...
          <div style={totalsCard}>
            <div style={totalGrid}>
              ...
              <StatBox
                ...
                value={formatCurrencyMap(supplierBalanceReductionByCurrency)}
                color="#dc2626"
              />
            </div>
        ...
        <main style={mainContent}>
          <div style={headerCard}>
            {headerSelector ? headerSelector : <h2 style={{ margin: 0 }}>گەڕانەوەی کڕین</h2>}
              ...
              {showProductList && !isLocked && (
                <div style={productDropdown}>
                  {filteredProducts.length === 0 ? (
                    <div style={emptyText}>هیچ کەرەستەی بەردەست نەدۆزرایەوە</div>
                  ) : (
                  ...
                  {rows.length === 0 ? (
                    ...
                    rows.map((row, index) => (
                      <tr key={row.id}>
                              ...
                              <>
                                ...
                                <div style={detailGrid}>
                                  <Field label="پێچانەوە">
                                    ...
                                    >
                                      {getProductPackages(
                                        products.find((p: any) => p.id === row.productId)
                                      ).map((pkg) => (
                        ...
                        {tableColumns.code && <td style={tdCenter}>{row.code || "-"}</td>}
                        ...
                        {tableColumns.qty && (
                          <td style={tdCenter}>
                            <input
                              ...
                              disabled={isLocked}
                              onChange={(e) => {
                                const nextQty = onlyDecimal(e.target.value);
                                const nextUnits = toNumber(nextQty) * row.packageQuantity;
          ...
          <div style={summaryCard}>
            ...
            <SummaryItem label="گشتی" value={formatCurrencyMap(returnTotalsByCurrency)} />
      ...
      <div id="purchase-return-print-area" style={printArea}>
        <div style={printPage}>
            ...
            <div style={printInfoGrid}>
              ...
              {printOptions.showInvoiceInfo ? (
                <div style={{ ...printInfoBox, width: "100%", minWidth: "220px" }}>
                  <PrintInfoLine label="جۆری پسوڵە" value="گەڕانەوەی کڕین" />
                  <PrintInfoLine label="ژمارەی پسوڵە" value={invoiceNumber} />
                  <PrintInfoLine label="بەروار" value={formatDate(invoiceDate)} />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%" }}>
                {printOptions.showSupplierInfo && (
                  <div style={{ ...printInfoBox, width: "100%", minWidth: "220px" }}>
                    ...
                    <PrintInfoLine
                      ...
                      value={getAccountBalancePrintText()}
                    />
                  </div>
                )}
                ...
                {printOptions.showEmployeeName || printOptions.showEmployeePhone && (employeeName || employeePhone) && (
                  <div style={{ ...printInfoBox, width: "100%", minWidth: "220px" }}>
                    <PrintInfoLine label="کارمەند" value={employeeName} />
                    <PrintInfoLine label="ژمارەی تەلەفۆن" value={employeePhone} />
                  </div>
                )}
              </div>
            </div>
          ...
          <table style={printTable}>
            <thead>
              <tr>
                ...
                {tableColumns.returnPrice && <th style={printTh}>نرخی گەڕانەوە</th>}
                {tableColumns.discount && rows.some((row: any) => toNumber(row.discount) > 0) && (
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr key={row.id}>
                  ...
                  {tableColumns.code && <td style={printTd}>{row.code || "-"}</td>}
          ...
          <div style={printItemCountLine}>
            <span>کۆی کەرەستەکان</span>
        ...
        <div style={modalOverlay}>
          <div style={confirmBox}>
            ...
            <div style={confirmActions}>
              <button style={outlineBlueBtn} onClick={() => setShowNewInvoiceConfirm(false)}>
                گەڕانەوە بۆ پسوڵە
              </button>
...
const pageGrid: CSSProperties = { display: "grid", gridTemplateColumns: "var(--page-grid-cols, 360px 1fr)", gap: 18, alignItems: "stretch" };
const leftPanel: CSSProperties = { background: "white", border: "1px solid #e5e7eb", borderRadius: 18, padding: 16, position: "sticky", top: 16 };
const mainContent: CSSProperties = { display: "flex", flexDirection: "column", gap: 16,
  minWidth: 0, };
...
const compactInput: CSSProperties = { width: "100%", padding: "6px 8px", borderRadius: 7, border: "1px solid #d1d5db", fontSize: 12, outline: "none", background: "white", boxSizing: "border-box", fontFamily: appFont };
const smallInput: CSSProperties = { width: 90, padding: "10px 8px", borderRadius: 10, border: "1px solid #d1d5db", textAlign: "center", outline: "none", fontFamily: appFont };
...
const compactTextarea: CSSProperties = { width: "100%", padding: "6px 8px", borderRadius: 7, border: "1px solid #d1d5db", fontSize: 12, outline: "none", resize: "vertical", boxSizing: "border-box", fontFamily: appFont };
const labelStyle: CSSProperties = { marginBottom: 6, fontWeight: 700, color: "#374151" };
const supplierInputWrap: CSSProperties = { position: "relative", width: "100%" };
const supplierClearBtn: CSSProperties = { position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)", width: 28, height: 28, borderRadius: "50%", border: "1px solid #fecaca", background: "#fee2e2", color: "#dc2626", fontSize: 20, fontWeight: 900, lineHeight: "24px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: appFont, zIndex: 5 };
...
const productSearchBox: CSSProperties = { position: "relative", marginBottom: 16 };
const productDropdown: CSSProperties = { position: "absolute", top: "100%", right: 0, left: 0, marginTop: 6, background: "white", border: "1px solid #d1d5db", borderRadius: 12, boxShadow: "0 14px 35px rgba(15,23,42,0.12)", zIndex: 70, maxHeight: "70vh", overflowY: "auto" };
const productDropdownItem: CSSProperties = { display: "block", width: "100%", textAlign: "right", border: 0, background: "white", padding: 12, cursor: "pointer", borderBottom: "1px solid #f1f5f9", fontFamily: appFont };
const emptyText: CSSProperties = { padding: 14, textAlign: "center", color: "#64748b" };
const emptyTableCell: CSSProperties = { padding: 26, textAlign: "center", color: "#64748b", borderBottom: "1px solid #eef2f7", fontWeight: 700 };
...
const currentBadge: CSSProperties = { background: "#eff6ff", color: "#1d4ed8", border: "1px solid #bfdbfe", borderRadius: 999, padding: "7px 14px", fontWeight: 800 };
...
const detailFooter: CSSProperties = { marginTop: 10, display: "flex", justifyContent: "flex-start" };
const blueSaveBtn: CSSProperties = { background: "#2563eb", color: "white", border: 0, borderRadius: 8, padding: "8px 18px", fontWeight: 900, cursor: "pointer", fontFamily: appFont, width: "100%" };
...
const modalBox: CSSProperties = { width: 1080, maxWidth: "98vw", height: "95vh", maxHeight: "98vh", overflowY: "auto", background: "white", borderRadius: 16, padding: 20, boxShadow: "0 25px 70px rgba(15,23,42,0.25)" };
