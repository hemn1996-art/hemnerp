...
export default function MyDebtPage({ headerSelector, editId }: Props) {
  ...
  const accountBalanceBeforeByCurrency = useMemo(() => {
    ...
    if (originalVoucher.ledgerEntries && originalVoucher.ledgerEntries.length > 0) {
      ...
      // For MyDebt (creditor), the voucher subtracted from balance, so we add it back to get before balance
  ...
  useEffect(() => {
    ...
    checkFn.owner = 'MyDebtPage.tsx';
    (window as any).hasUnsavedChanges = checkFn;
    return () => {
      if ((window as any).hasUnsavedChanges && (window as any).hasUnsavedChanges.owner === 'MyDebtPage.tsx') {
  ...
  function handlePrint(size: "A4" | "A5" = "A4") {
    ...
    openPrintWindow("my-debt-print-area");
  }
