...
export default function DebtDiscountPage({ headerSelector, editId }: Props) {
  ...
  const hasIqdDiscount = getCurrencyCode(discountCurrencyId) === "IQD";
  ...
  const discountInDefaultCurrency = convertCurrency(
  ...
  useEffect(() => {
    ...
    checkFn.owner = 'DebtDiscountPage.tsx';
    (window as any).hasUnsavedChanges = checkFn;
    return () => {
      if ((window as any).hasUnsavedChanges && (window as any).hasUnsavedChanges.owner === 'DebtDiscountPage.tsx') {
  ...
  function getDiscountByCurrency() {
    if (toNumber(discountAmount) <= 0) return {};

    const normId = getNormalizedCurrencyId(discountCurrencyId);
    return {
      [String(normId)]: toNumber(discountAmount),
    };
  }
  ...
  function validateBeforeSave() {
    if (!accountId) {
      showToast("پسوڵەی داشکاندن نابێت بێ هەژمار خەزن بکرێت.");
      return false;
    }

    if (toNumber(discountAmount) <= 0) {
      showToast("تکایە بڕی داشکاندن داغڵ بکە.");
      return false;
    }
  ...
  function handleSave() {
    ...
    const payload = {
      ...
      total: discountInDefaultCurrency,
      totalByCurrency: getDiscountByCurrency(),

      discountAmount: toNumber(discountAmount),
      discountCurrencyId,
      discountByCurrency: getDiscountByCurrency(),
      ...
      // تەواوی داشکاندنەکە دەبێتە قازانج
      profitEffect: discountInDefaultCurrency,
    ...
    showToast("پسوڵەی داشکاندن خەزن کرا ✅", "success");
  }
  ...
  return (
    <div style={page}>
              ...
              <StatBox
                ...
                value={
                  toNumber(discountAmount) > 0
                    ? formatCurrencyAmountJSX(toNumber(discountAmount), discountCurrencyId)
                    : "0"
                }
                color="#16a34a"
              />
      ...
      <div id="debt-discount-print-area" style={printArea}>
        <div style={printPage}>
          <PrintWatermark />
          <PrintHeader />
            ...
            <div style={printInfoGrid}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, width: "100%" }}>
                ...
                {/* Employee Info Box */}
                {printOptions.showEmployeeInfo && (employeeName?.trim() !== "" || employeePhone?.trim() !== "") && (
                  <div style={{ ...printInfoBox, width: "100%", minWidth: "220px" }}>
                                        {employeeName?.trim() !== "" && (
                      <PrintInfoLine
                        label="کارمەند"
