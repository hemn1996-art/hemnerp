...
// Find start and end indices dynamically
const startIndex = lines.findIndex(line => line.includes('<button') && line.includes('key={account.id}'));
const endIndex = lines.findIndex(line => line.includes('<InfoRow label="کەفیل">'));

if (startIndex !== -1 && endIndex !== -1 && endIndex > startIndex) {
    // go back one line from start index to include `<button` properly since my check was on `key={account.id}` line
    let actualStart = lines[startIndex - 1].includes('<button') ? startIndex - 1 : startIndex;
    if (lines[startIndex].includes('<button')) actualStart = startIndex;

    lines.splice(actualStart, endIndex - actualStart + 1, ...goodLines);
